...
export function CorgiMascot({ stage = "adult", condition = "happy", size = 160, bob = false, style }: CorgiMascotProps) {
  const OL="#5A3A24",TAN="#EDA94C",CREAM="#FBF1E0",INNER="#FBE3C9",DARK="#4A2E1C",BLUSH="#F6B7BD",TONGUE="#F4839A",COLD="#A9CBDD",GOLD="#F2C14E",GOLD_D="#B8860B",SKY="#7FB7D6",CORAL="#E2574C",CORAL_D="#A8392F",LEAF="#7FB069",LEAF_D="#4E7A3F",SKY_D="#4E97C2";
  ...
  const Dog=(
    <g transform={`translate(200 250) scale(${SC}) translate(-200 -250)`}>
      ...
      <path className="m-ol" fill={TAN} d="M126 96 L92 18 L196 86 Z"/>
      <path className="m-ol" fill={TAN} d="M274 96 L308 18 L204 86 Z"/>
      <path fill={INNER} d="M138 92 L118 42 L182 86 Z"/>
      <path fill={INNER} d="M262 92 L282 42 L218 86 Z"/>
      ...
      <ellipse cx="138" cy="178" rx="26" ry="20" fill={c.cheek}/>
      <ellipse cx="262" cy="178" rx="26" ry="20" fill={c.cheek}/>
      {Eyes}{Mouth}{(condition==="down"||condition==="cold")&&Tears}{c.aura==="sweat"&&Sweat}{stage==="puppy"&&Sprout}{stage==="legend"&&Crown}
    </g>
  );
  return (
