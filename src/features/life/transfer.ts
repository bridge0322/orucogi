// データの引き継ぎ（バックアップ／復元）。
// 積立記録（orucogi_personal_v1）と生活データ（orucogi_life_v1）を
// 1つの「ひきつぎコード」にまとめる。サーバ不要・端末間はコピペ or ファイルで運ぶ。
// 形式: "ORUX1." + base64(gzip(JSON))。gzip非対応環境では "ORUX0." + base64(JSON)。
import { dayKey } from "./time";

const KEYS = { personal: "orucogi_personal_v1", life: "orucogi_life_v1" } as const;
...
export async function makeTransferCode(): Promise<string> {
  const payload: Record<string, unknown> = { app: "orucogi", v: 1, at: new Date().toISOString() };
  try {
...
export function downloadTransferCode(code: string) {
  downloadBlob(new Blob([code], { type: "text/plain" }), `orucogi-hikitsugi-${dayKey()}.txt`);
}
...
export function downloadBackupJson() {
  ...
  const name = `orucogi_backup_${dayKey().replaceAll("-", "")}.json`;
