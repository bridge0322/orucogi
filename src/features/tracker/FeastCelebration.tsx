...
import { pickThanks } from "./logic/feast";
...
export function FeastCelebration({ food, amount, gainedXp, onDone }: FeastCelebrationProps) {
  const [phase, setPhase] = useState(0); // 0:登場 1:もぐもぐ 2:よろこぶ
  const [msg] = useState(pickThanks);
  useEffect(() => {
    ...
    <div onClick={phase >= 2 ? onDone : undefined}
      ...
      <div style={{ position: "relative", width: 240, height: 240, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {happy && [0, 1, 2, 3, 4].map((i) => (
          <span key={i} style={{ position: "absolute", left: `${18 + i * 16}%`, bottom: "38%", fontSize: 22 + (i % 3) * 6, animation: `heart-up ${1.1 + (i % 4) * 0.25}s ease-out ${i * 0.12}s infinite` }}>💛</span>
        ))}
      ...
      <div style={{ fontFamily: "var(--font-display)", fontWeight: 900, fontSize: "var(--text-3xl)", color: "var(--text-strong)", marginTop: 10, minHeight: 46 }}>
        {happy ? msg : "もぐもぐ…"}
      </div>
      {happy && (
        <>
          <div style={{ fontFamily: "var(--font-body)", fontSize: "var(--text-md)", color: "var(--text-body)", marginTop: 4 }}>
            {food.name}で <b style={{ color: "var(--text-brand)" }}>¥{YEN(amount)}</b> つみたて！
          </div>
